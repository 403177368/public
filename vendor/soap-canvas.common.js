...
Canvas.prototype._init_cv = function _init_cv (cvOpts) {
  ...
  for (var key$1 in props) {
    cv[key$1] = props[key$1];
  }
  for (var key$2 in cvOpts.props) {
    cv[key$2] = cvOpts.props[key$2];
  }
  ...
  for (var key$3 in cvOpts.config) {
    cv._config[key$3] = cvOpts.config[key$3];
  }
...
// Render current fps on top-left of the canvas per second 
